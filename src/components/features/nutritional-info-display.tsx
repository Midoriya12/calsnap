...
import type { AIEstimation, IngredientNutritionInfo, NewLoggedMealData } from '@/types';
import { Card, CardContent, CardHeader, CardTitle, CardFooter } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Separator } from '@/components/ui/separator';
import { ShoppingCart, Info, ListTree, Utensils, Lightbulb, ClipboardList, Zap, Loader2, AlertCircle, Clock, Users, ChefHat, BookOpen, ListChecks, Save, ClipboardCopy, FilePlus2 } from 'lucide-react';
...
export function NutritionalInfoDisplay({ estimation, uploadedImage }: NutritionalInfoDisplayProps) {
  ...
  const handleOpenLogMealForm = () => {
    if (!user) {
      toast({ title: "Login Required", description: "Please log in to log your meals.", variant: "destructive" });
      return;
    }
    setIsLogMealFormOpen(true);
  };
          ...
          <CardTitle className="flex items-center justify-between text-xl font-headline text-primary">
            ...
            <div className="flex items-center gap-2">
              {user && uploadedImage && estimation && (
               ...
               {user && estimation && (
                <Button onClick={handleOpenLogMealForm} variant="outline" size="sm">
